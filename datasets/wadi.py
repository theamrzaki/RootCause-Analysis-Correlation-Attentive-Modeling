...
class WADI:
    ...
    def load_data(self):
        ...
        if not self.data_dir_path_modified_with_window_var:
            self.data_dir = os.path.join(self.data_dir, f"window_{self.window_size}_vars_{self.num_vars}")
        self.data_dict['x_n_list'] = None#np.load(os.path.join(self.data_dir, 'x_n_list.npy'))
    ...
    def apply_orthogonal_transform(self, save_path, device='cpu'):
        ...
        with torch.no_grad():
            self.data_dict['x_n_orth'] = None#self.orth_transformer(x_n_tensor).cpu().numpy()
    ...
    def pipeline_sanity_check(self):
        print("\n--- Starting WADI Data Pipeline Sanity Check ---")
