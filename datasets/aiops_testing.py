...
class aiops:
    ...
    def get_wide_table(self):
        ...
        for d_dir in tqdm(date_dirs, desc="Processing Date Folders"):
            ...
            for m_type in self.metric_types:
                ...
                for f in tqdm(csv_files, desc="Processing CSV Files", leave=False):
                    ...
                    try:
                        ...
                        if not {"timestamp", "cmdb_id", "value"}.issubset(df.columns):
                            continue
                        ...
                        pivoted.columns = [f"{c}_{metric_name}" for c in pivoted.columns]
            ...
            # MERGE METRICS
            # =====================================================
            day_modality = {"metrics": None, "logs": None, "traces": None}

            if day_metric_frames:
                day_modality["metrics"] = pd.concat(
                    day_metric_frames, axis=1, copy=False, join="outer"
                )
            ...
            if self.include_logs_and_traces:
                ...
                if log_features is not None:
                    for c in log_features.columns:
                        log_features[c] = log_features[c].astype(
                            pd.SparseDtype("float32", np.nan)
                        )
                ...
                if trace_features is not None:
                    for c in trace_features.columns:
                        trace_features[c] = trace_features[c].astype(
                            pd.SparseDtype("float32", np.nan)
                        )
            ...
            # CONCAT MODALITIES
            # =====================================================
            parts = [v for v in day_modality.values() if v is not None]
            ...
            day_wide = pd.concat(parts, axis=1, copy=False, join="outer")
            day_wide.columns = day_wide.columns.astype(str)

            print(f"Saving pickle: {temp_path} shape={day_wide.shape}")
            day_wide.to_pickle(temp_path)

            del parts, day_modality, day_wide
            gc.collect()
        ...
        pickle_files = sorted(
            glob(os.path.join(self.data_dir, f"temp_window{self.window_size}_2022-*.pkl"))
        )
        ...
        for pf in pickle_files:
            ...
            for c in df.columns:
                if not isinstance(df[c].dtype, pd.SparseDtype):
                    df[c] = df[c].astype(pd.SparseDtype("float32", np.nan))

            all_date_frames.append(df)

        # =========================================================
        # STEP 3: FINAL CONCAT (UNIX SECONDS INDEX)
        # =========================================================
        print("Finalizing global alignment...")

        full_df = pd.concat(all_date_frames, axis=0, copy=False, sort=False)

        # DO NOT touch index
        self.time_index = full_df.index.to_numpy(dtype=np.int64)

        full_df = full_df.reset_index(drop=True)

        # FORCE: unix seconds index
        full_df.index = full_df.index.astype(np.int64) // 1_000_000_000
    ...
    def parse_json_groundtruth(self, full_df):
        ...
        label_matrix = np.zeros((len(self.time_index), full_df.shape[1]), dtype=np.float32)
        ...
        # SAFE time axis (already UNIX seconds)
        time_index = self.time_index  # NOT full_df.index

        # sanity check
        assert time_index.max() < 10**12, "Index is not in seconds!"
        ...
        for f in gt_files:
            ...
            for i in range(len(timestamps)):
                ...
                onset = int(timestamps[i])
                cmdb = cmdb_ids[i]

                faulty_indices = cmdb_to_indices.get(cmdb, [])
                if not faulty_indices:
                    continue

                # bin conversion (1-minute bins)
                onset_bin = onset // 60
                ...
                time_mask = (
                    (time_index >= onset) &
                    (time_index <= onset + duration)
                )
                rows = np.flatnonzero(time_mask)
                cols = np.asarray(faulty_indices)

                label_matrix[np.ix_(rows, cols)] = 1
    ...
    def generate_example_metrics_only(self):
        ...
        df_subset.index = df_subset.index.astype(np.int64)
        # 3. Label Processing
    ...
    def generate_example_metrics_logs_traces(self):
        ...
        # =========================================================
        # 0. FORCE UNIX SECONDS INDEX (CRITICAL FIX)
        # =========================================================
        df.index = pd.to_datetime(df.index, unit="s")
        df = df.sort_index()

        df.index = (df.index.astype("int64") // 1_000_000_000).astype(np.int64)
        self.start_ts = int(df.index.min())
        ...
        log_df = df[log_cols] if log_cols else None
        trace_df = df[trace_cols] if trace_cols else None

        # =========================================================
        # 2. FEATURE SELECTION (UNCHANGED LOGIC)
        # =========================================================
        sample = metric_df.iloc[:1000].to_numpy(dtype=np.float32)
        ...
        # =========================================================
        # 3. LOG/TRACE REDUCTION
        # =========================================================
        if log_df is not None:
            std = np.nanstd(log_df.to_numpy(dtype=np.float32), axis=0)
            keep = std > 1e-6
            cols = np.array(log_df.columns)[keep]
            log_df = log_df[cols[:self.options.get("log_features", 10)]]

        if trace_df is not None:
            std = np.nanstd(trace_df.to_numpy(dtype=np.float32), axis=0)
            keep = std > 1e-6
            cols = np.array(trace_df.columns)[keep]
            trace_df = trace_df[cols[:self.options.get("trace_features", 10)]]
        ...
        ts = df.index.to_numpy(dtype=np.int64)
        bin_id = ts // 60
        ...
        metric_df.index = metric_df.index.astype(np.int64)
        if log_df is not None:
            log_df.index = log_df.index.astype(np.int64)
        if trace_df is not None:
            trace_df.index = trace_df.index.astype(np.int64)
        ...
        print("Final shape:", df_subset.shape)
        ...
        test_labels = label_matrix[split:]
        ...
        for i in range(len(train) - self.window_size):
            x_n.append(train[i:i+self.window_size])

        for i in range(len(test) - self.window_size):
            x_ab.append(test[i:i+self.window_size])
            y_ab.append(test_labels[i:i+self.window_size])

        self.data_dict["x_n_list"] = np.array(x_n)
        self.data_dict["x_ab_list"] = np.array(x_ab)
        self.data_dict["label_list"] = np.array(y_ab)

        print("Dataset generated:",
            self.data_dict["x_n_list"].shape,
            self.data_dict["x_ab_list"].shape)

        self.save_data()

        ortho_dir = os.path.join(self.data_dir, "orth_transform_meta")
