...
def run_multi_phase(normal_df, anomalous_df, gamma, localized, bins, verbose):
    """
    Run RCD with numerical scoring.
    Primary score: MI accumulation
    Fallback score: survival depth
    """
    ...
    survival_scores = defaultdict(int)
    ...
    while True:
        ...
        # Increase survival score for surviving nodes
        for v in f_child_union:
            survival_scores[v] += 1

        f_child_union, mi, ci = run_level(
        ...
        if verbose:
            print(
                f"Level-{level}: variables {len(f_child_union)} | "
                f"time {time.time() - start:.2f}s"
            )

        level += 1

        # Accumulate MI if available
        for item in mi:
            try:
                var, val = item
                mi_scores[var] += float(val)
            except Exception:
                pass  # MI not usable → ignore safely

        if isinstance(ci, (int, float)):
            total_ci_tests += ci

        curr_len = len(f_child_union)
        if curr_len <= gamma or curr_len == prev:
            break
        prev = curr_len
    ...
    for v in rc:
        if mi_scores[v] > 0:
            rc_scores[v] = mi_scores[v]
        else:
            rc_scores[v] = survival_scores[v]

    rc_sorted = sorted(rc_scores, key=rc_scores.get, reverse=True)

    return {
        "ranks": rc_sorted,
        "scores": rc_scores,
        "ci_tests": total_ci_tests,
    }
