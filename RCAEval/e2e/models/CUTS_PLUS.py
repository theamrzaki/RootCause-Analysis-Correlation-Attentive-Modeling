...
class Model(nn.Module):
    def __init__(self, model_config):# num_vars, order, hidden_dim=128, n_layers=1, device="cpu", options=None):#128 for SMD
        super().__init__()

        num_vars = model_config.enc_in
        order = model_config.seq_len
        hidden_dim = model_config.d_model
        n_layers = model_config.e_layers
