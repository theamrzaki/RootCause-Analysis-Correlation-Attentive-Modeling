...
def causalrca(data, inject_time=None, dataset=None, with_bg=False, **kwargs):
    ...
    model_size_mb = (
        sum(p.numel() * p.element_size() for p in encoder.parameters()) +
        sum(p.numel() * p.element_size() for p in decoder.parameters())
    ) / (1024 ** 2)
        ...
        for step_k in range(k_max_iter):
            ...
            while c_A < 1e20:
                for epoch in range(CONFIG.epochs):
                    ...
                    # patience logic
                    # ----------------
                    current = loss_key(ELBO_loss)
                    best = loss_key(best_loss)
